...
impl<S: IStorage, const N: usize> BlockBufferStorage<S, N> {
    ...
    fn get_aligned(
        ...
        let result = (leading, aligned, trailing);
        log::debug!("aligned offset={original_offset}, len={original_len} to {result:?}");
        result
    }
    ...
    fn cache_aligned_single_sector_read(
        ...
        Ok(buf.copy_from_slice(&cache[offset_in_sector_buf..][..core::cmp::min(N, buf.len())]))
    }
}
...
impl<S: IStorage, const N: usize> IStorage for BlockBufferStorage<S, N> {
    fn read_at(&self, offset: u64, buf: &mut [u8]) -> crate::SwonchResult<u64> {
        if offset >= self.length()? || buf.len() == 0 {
            return Ok(0);
        }
